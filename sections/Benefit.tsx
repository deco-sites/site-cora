...
export default function Benefit({ title, ItenList }: BenefitProps) {
  return (
    <div
      className={"bg-secondary py-12 px-6 flex flex-col -mt-14 xl:grid lg:grid-cols-12 lg:px-16 lg:gap-6 md:mt-0"}
    >
      <div className={"lg:col-start-2 lg:col-end-6"}>
        <h2
          className={"text-base-100 text-3xl pr-8 sm:text-[2.6rem] leading-8 sm:mb-6 lg:text-[3.5rem] lg:leading-[3.6rem] lg:pr-8"}
        >
          {title.simple}
          <span className={"text-primary"}>{title.color}</span>
        </h2>
      </div>
      <div className={"py-7 lg:col-start-6 lg:col-end-[-2]"}>
        <ul
          className={"grid grid-cols-1 grid-rows-4 sm:grid-cols-2 sm:grid-rows-2 gap-6 justify-between"}
        >
          {ItenList.map((iten) => {
            return (
              <div className={"flex flex-row gap-3 w-full"}>
                ...
                <li
                  className={"text-base-100 font-light lg:font-normal lg:text-[1.1rem]"}
                >
                  {iten.label}
                </li>
              </div>
